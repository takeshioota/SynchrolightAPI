...
public class EffectRunnerService
{
        ...
        CancellationToken ct)
    {
        ...
        // BUG-20260901-04: Flash→Rainbow 遷移で、前エフェクトのピーク色が端末にラッチされたまま
        // 下の「パレット反映待ち(50ms)」の間 残り「一瞬フリーズ＋前色残り」に見える不具合の対策。
        // パレット送信の前に高優先の黒(消灯)を1発入れ、端末が保持中の残色を即座に打ち消す。
        // ダイレクト経路のためグローバル場次(0x01)を使用（色/エフェクト経路と同一）。
        await _transport.EnqueueAsync(
            LightProtocol.BuildA2_GlobalColor(0x01, 0, 0, 0), highPriority, effectCt);
        ...
        while (!effectCt.IsCancellationRequested)
        {
            ...
            if (target != lastFrame)
            {
                ...
                _logger.LogDebug("Rainbow frame→{Frame} at {Elapsed}ms", target, now);
            }
            else if (now - lastSendMs >= RainbowRefreshMs)
            {
                ...
                // 維持送信し、セルフモード防止だけ行って包絡を乱さない。フェード開始は各フレーム変化時の
                // A9 03 が担い、その色の包絡は次のフレーム変化まで途切れずに走る。
            ...
            {
                _logger.LogInformation(
                    "Rainbow 送信要約: 経過={Sec}s, フレーム変化={Frames}, 送信パケット={Packets}, cycle={Cycle}ms",
    ...
    /// <summary>
    /// 直近に送出した単色(A2)を取得する（20260904）。
    /// フェード等の停止時に「その瞬間の光度」で保持するため、UI が停止直前に参照する。
    /// </summary>
    public bool TryGetCurrentColor(out byte field, out Rgb color)
        => _scheduler.TryGetLastColor(out field, out color);
